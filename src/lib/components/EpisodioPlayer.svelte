...
<div class="flex items-center justify-between text-sm">
  <span class="font-semibold text-oceano">
    Folge {episodio.numero}{episodio.parte ? ` · ${episodio.parte}` : ''}
  </span>
...
<div class="card mt-4 flex min-h-[220px] flex-col items-center justify-center gap-3 p-5 text-center">
  ...
  {#if fase === 'pausa' && step?.tipo === 'responde'}
    ...
    <p class="text-2xl font-extrabold text-terracota">🎤 Jetzt sprechen!</p>
    {#key index}
  ...
  {:else if fase === 'aguardando' && step?.tipo === 'responde'}
    ...
    {:else}
      <p class="text-sm text-carvao/50">Gesagt? Hör das Original — oder mach einfach weiter.</p>
    {/if}
    {#if recUrl}
      <button class="btn bg-oceano text-white text-sm" onclick={ouvirVoce}>▶ dich anhören</button>
    {/if}
  {:else if fase === 'pausa' && step?.tipo === 'shadow'}
    <p class="text-2xl font-extrabold text-terracota">🗣️ Sprich mit!</p>
    <p class="text-sm text-carvao/50">versuch es ohne zu lesen</p>
  {:else if ehPt}
  ...
  {#if erroAudio}
    <p role="status" class="text-xs font-medium text-terracota">
      ⚠️ Audio nicht verfügbar — prüf deine Verbindung (oder speicher offline, bevor du losfährst).
    </p>
  {/if}
</div>
...
<div class="mt-4 flex items-center justify-center gap-3">
  <button
    ...
    onclick={anterior}
    aria-label="Vorheriger Schritt">⏮</button
  >

  {#if fase === 'aguardando'}
    <button class="btn-primary px-6 py-3" onclick={ouvirResposta}>▶ Antwort hören</button>
    <button class="btn bg-salvia px-6 py-3 text-white" onclick={proximo}>⏭ weiter</button>
  {:else}
    <button class="btn-primary px-8 py-3 text-lg" onclick={botaoCentral}>
      {#if fase === 'parado'}{index === 0 ? '▶ Los' : '▶ Weiter'}
      {:else if fase === 'fim'}↺ Nochmal
      {:else}⏸ Pause{/if}
    </button>
  {/if}
  ...
  <button
    class="btn bg-white text-lg ring-1 ring-black/10"
    onclick={proximo}
    aria-label="Nächster Schritt">⏭</button
  >
</div>

<div class="mt-3 flex flex-wrap items-center justify-center gap-4 text-sm">
  <button class="-my-2 px-2 py-3 text-oceano" onclick={repetir}>⟲ Schritt wiederholen</button>
...
<div class="mt-3 flex justify-center">
  ...
  >
    {#if baixando}⬇ lädt… {baixaProg}/{clipKeys.length}
    {:else if baixado}✅ offline gespeichert
    {:else}⬇ diesen Teil offline speichern{/if}
  </button>
</div>
...
{#if fase === 'fim'}
  <p class="mt-5 text-center text-lg font-semibold text-salvia">✅ Teil geschafft!</p>
{/if}
