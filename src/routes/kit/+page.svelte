  ...
  {#snippet rodape()}
    <div class="flex items-end gap-2">
      ...
      {#if chaves.length}
        ...
        >
          ...
          {:else if estado === 'pronto'}✓ Reise
          {:else if estado === 'parcial'}↻ Reise
          {:else if estado === 'sem-suporte'}— Reise
          {:else}⬇ Reise{/if}
        </button>
      {/if}
    </div>
    {#if estado === 'parcial'}
      <p role="status" class="mt-1 text-center text-[0.72rem] font-semibold text-terracota">
        Ein paar Clips sind nicht durchgekommen — tipp nochmal auf ↻ Reise, solang du Empfang hast.
      </p>
    {:else if estado === 'sem-suporte'}
      <p role="status" class="mt-1 text-center text-[0.72rem] font-semibold text-terracota">
        Dieser Browser kann das Audio nicht speichern. Leg die App zuerst auf den Startbildschirm.
      </p>
    {/if}
  {/snippet}
</KitTela>
