  ...
  <div class="min-h-0 flex-1 space-y-2.5 overflow-y-auto overscroll-contain pb-1">
    ...
    {:else}
      ...
      <div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-black/10">
        <p class="text-[1.05rem] font-bold leading-snug">Hier ist noch nichts.</p>
        <p class="mt-1 text-[0.9rem] leading-snug text-carvao/75">
          Der Punkt steht auf der Karte, aber die Sätze dafür sind noch nicht im Spickzettel
          gelandet. Der Rest von {data.tile.rotulo} hat vielleicht schon, was du brauchst.
        </p>
        <a
        ...
        >
          <span class="text-oceano">◀</span> Zurück zu {data.tile.rotulo}
        </a>
      </div>
    {/if}
  </div>
